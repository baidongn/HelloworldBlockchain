...
public class StackBasedVirtualMachine {
    ...
    public ScriptExecuteResult executeScript(Transaction transactionEnvironment, Script script) throws RuntimeException {
        ...
        for(int i=0;i<script.size();i++){
            ...
            if(HexUtil.bytesToHexString(OperationCodeEnum.OP_DUP.getCode()).equals(command)){
                stack.push(stack.peek());
            }else if(HexUtil.bytesToHexString(OperationCodeEnum.OP_HASH160.getCode()).equals(command)){
                String top = stack.peek();
                String publicKeyHash = AccountUtil.publicKeyHashFromPublicKey(top);
                stack.pop();
                stack.push(publicKeyHash);
            }else if(HexUtil.bytesToHexString(OperationCodeEnum.OP_EQUALVERIFY.getCode()).equals(command)){
                if(!stack.pop().equals(stack.pop())){
                    throw new RuntimeException("脚本执行失败");
                }
            }else if(HexUtil.bytesToHexString(OperationCodeEnum.OP_CHECKSIG.getCode()).equals(command)){
            ...
            }else if(HexUtil.bytesToHexString(OperationCodeEnum.OP_PUSHDATA.getCode()).equals(command)){
